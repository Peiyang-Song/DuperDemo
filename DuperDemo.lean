...
example (a b c : Nat) : a + b + c = a + c + b := by
  ...
  -- duper -- Duper failed to solve the goal and determined that it will be unable to
           -- do so with the current configuration of options and selection of premises.

  -- duper [Nat.add_assoc] -- (deterministic) timeout at 'whnf', maximum number of
                           -- heartbeats (200000) has been reached (use 'set_option
                           -- maxHeartbeats <num>' to set the limit)

  -- duper [Nat.add_assoc, Nat.add_comm] -- Duper succeeds.
